...
def create_speech_silence_speech(source_path: str, silence_sec: float, out_path: str) -> str:
    ...
    print(f"  Created speech-silence-speech: {out_path} ({total_dur:.1f}s, "
    ...
    if tensors:
        ...
        for spk_idx in range(probs.shape[2]):
            ...
            if active > 0 or max_val > 0.1:
                print(f"    Speaker {spk_idx}: max={max_val:.3f}, mean={mean_val:.3f}, "
                      f"active={active}/{total} frames")
    else:
        print(f"  No tensor output")
...
def main():
    ...
    gap_path = create_speech_silence_speech(source_path, 15.0, f"{tmp_dir}/speech_gap_speech.wav")
    ...
    for name, (segs, tensors, text) in results.items():
        ...
        if tensors:
            probs = tensors[0]
            active_spks = sum(
                1 for i in range(probs.shape[2])
        ...
        print(f"\n  {name}:")
        print(f"    Segments: {num_segs}, Active speakers: {active_spks}, Words: {num_words}")
    ...
    if mono_tensors:
        ...
        if active_count > 1:
            print(f"    WARNING: Sortformer hallucinated extra speaker(s) from a monologue!")
        else:
            print(f"    Good: single speaker correctly identified")
    ...
    if gap_segs and gap_segs[0]:
        ...
        if gap_segments:
            print(f"    WARNING: {len(gap_segments)} segment(s) detected during silence gap!")
        else:
            print(f"    Good: silence gap is clean")
