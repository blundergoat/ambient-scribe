...
for method_name in ['_transcribe_forward', '_transcribe_input_processing', '_transcribe_on_begin']:
...
with torch.inference_mode():
    tensor_output = diar.diarize(audio=audio_path, batch_size=1, include_tensor_outputs=True)
        ...
        if isinstance(item, list):
            ...
            if len(item) > 0:
                ...
                if isinstance(item[0], torch.Tensor):
                    ...
                    print(f"      [0] len={len(item[0])}, first={item[0][:3] if item[0] else 'empty'}")
                else:
                    s = str(item[0])
                    print(f"      [0] = {s[:200]}")
            ...
            for k, v in item.items():
                if isinstance(v, torch.Tensor):
                    ...
                    print(f"        {k}: list of {len(v)} tensors, [0].shape={v[0].shape}")
                else:
                    s = str(v)
                    print(f"        {k}: {s[:200]}")
