...
def main():
    ...
    for dur in durations:
        ...
        print(f"  Diar: {dr['time']:.2f}s ({dr['segments']} segments, "
              f"RTF={dr['time']/dur:.4f})")

        ar = benchmark_asr(asr_model, path, label)
        asr_results.append(ar)
        print(f"  ASR:  {ar['time']:.2f}s ({ar['words']} words, "
    ...
    for i, dur in enumerate(durations):
        ...
        if prev_total and durations[i-1] > 0:
            dur_ratio = dur / durations[i-1]
            time_ratio = t / prev_total
            scale = f"{time_ratio:.2f}x"
        else:
            scale = "—"
        prev_total = t
        print(f"{dur:>8}s | {d:>10.2f} | {a:>10.2f} | {t:>10.2f} | {rtf:>8.4f} | {scale:>8}")
    ...
    if len(diar_results) >= 2:
        ...
        if ratio < 1.5:
            ...
            print("     Re-processing full audio each chunk is feasible for consultations.")
        elif ratio < 3.0:
            print("\n  => SCALING IS SUPER-LINEAR. Consider hybrid approach:")
            print("     Growing buffer for short sessions, switch to sliding window after N minutes.")
        else:
            print("\n  => SCALING IS QUADRATIC+. Sliding window required.")
            print("     Cannot re-process full audio — processing time will exceed real-time.")
    ...
    if over_90:
        diar_90_ok = all(
            diar_results[durations.index(d)]['segments'] > 0
            for d in over_90
        )
        if diar_90_ok:
            print(f"  Audio >90s diarized OK: segments found at {over_90}")
        else:
            print(f"  WARNING: Audio >90s may have diarization issues")
