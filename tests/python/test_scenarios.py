...
class TestScenarioFixtureIntegrity:
    ...
    def test_each_scenario_has_required_fields(self, scenarios):
        for s in scenarios:
            assert "id" in s, "Scenario missing id"
            assert "name" in s, f"Scenario {s.get('id')} missing name"
            assert "events" in s, f"Scenario {s['id']} missing events"
            assert "expectedEndState" in s, (
                f"Scenario {s['id']} missing expectedEndState"
            )
        ...
        for s in scenarios:
            for i, event in enumerate(s["events"]):
                ...
                assert "speaker_id" in data, (
                    f"Scenario {s['id']} segment {i} missing speaker_id"
                )
    ...
    def test_expected_end_state_has_segment_count(self, scenarios):
        for s in scenarios:
            ...
            assert "segmentCount" in expected, (
                f"Scenario {s['id']} missing segmentCount"
            )
            assert isinstance(expected["segmentCount"], int), (
                f"Scenario {s['id']} segmentCount not int"
            )
    ...
    def test_max_duration_is_positive(self, scenarios):
        for s in scenarios:
            ...
            if max_dur is not None:
                assert max_dur > 0, (
                    f"Scenario {s['id']}: maxDurationMs must be positive"
                )
