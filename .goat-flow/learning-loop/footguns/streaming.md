...
- **Evidence:** GPU-contact debugging on 2026-07-06 established the first two traps and exposed timestamp ambiguity. Span-fidelity Phase A recorded 1,443/1,443 hypotheses carrying token timestamps and 3,058 clamp calls. Phase B's sole full candidate recorded zero estimates but a 381.52-second final row against diarizer activity through 556.88 seconds, and strict placement fell from 86.85% to 53.78%. Those decision receipts were local-only; the durable anchors are in the installed producer path — semantic anchors `perform_parallel_streaming_stt_spk`, `active_speakers`, and `update_asr_state`, with the offset computation at `start_time = offset`. Re-read those in the installed NeMo package rather than trusting a recorded receipt.
...
- **Update (2026-07-10, chunk-cadence diagnosis complete):** the browser contract is 5000ms,
  ...
  rows to the frozen baseline. Against saved browser controls, free-oracle differences were 0.0pp/+1.6pp
...
- **Bounded-release result (2026-07-14):** a repeatedly refreshed mutable tail can pin the
