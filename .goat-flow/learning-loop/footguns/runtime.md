...
- **Evidence:** The 2026-07-30 disposition attempt began with the compose Mercure
  ...
  during fixture one. The terminal record was a local-only artifact; the failure signature
  was `first_fixture_websocket_keepalive_timeout`.
...
- **What breaks:** An earlier effort tried to quantify an overlap ceiling by transcribing PriMock57 doctor/patient source channels independently. Full-file batch transcription hit `torch.OutOfMemoryError` on c04, and browser-like WebSocket streaming of all separated channels later produced `websocket.error` rows from NeMo internals (`KeyError` in Sortformer diarization, then ASR `Cannot unfreeze partially without first freezing the module with freeze()`). After those errors, the shared singleton model needed a `docker compose restart nemo-agent` before normal eval could be trusted.
...
- **Update (2026-07-07, confidence spike):** confidence mode is NOT similarly cursed - enabling `confidence_cfg` (preserve word+token) on the same multitalker model, with the runtime's CUDA-graph workaround mirrored, ran clean on two fixtures with non-degenerate values and an error-free follow-up eval (`scripts/probe-word-confidence.py`). The timestamp caution stands; do not generalize it to confidence.

## Footgun: Unified word-confidence aggregation can veto a decoded correction chunk

**Status:** active | **Created:** 2026-07-24 | **Evidence:** ACTUAL_MEASURED
**Decision changed:** Treat a generic post-visit `transcribe_failed` as a possible NeMo confidence-aggregation failure; capture the exact failed chunk in an isolated cold call before attributing it to CUDA or accumulated model state. A successful retry with word confidence disabled is not sufficient proof when native word timestamps still fail the application one-to-one join.
**Trigger phase:** VERIFY
**Incident count:** 6 | **Latest occurrence:** 2026-07-24

- **Files:** `strands_agents/nemo_confidence.py` (search: `"preserve_word_confidence": True`)
- **Files:** `strands_agents/nemo_confidence.py` (search: "def disable_word_confidence_decoding")
- **Files:** `strands_agents/post_visit_correction.py` (search: "enable_word_confidence_decoding")
- **Files:** `strands_agents/post_visit_correction.py` (search: "def _word_confidence_aggregation_mismatch")
- **Files:** `strands_agents/post_visit_correction.py` (search: "def _transcription_from_hypothesis")
- **Files:** `strands_agents/post_visit_word_timing.py` (search: "def reconcile_punctuation_only_word_timings")
- **Files:** `strands_agents/post_visit_word_timing.py` (search: "def validated_word_timings")
- **What breaks:** The installed NeMo SentencePiece confidence aggregator runs inside `asr_model.transcribe()` and requires exactly one aggregate confidence per decoded word. On the frozen d2c09 tail it decoded 213 words but built 214 confidence values, then raised `RuntimeError` before returning a hypothesis. The application's defensive `word_confidences_for_display_words()` and `_transcription_from_hypothesis()` guards therefore cannot discard only the misaligned confidence evidence; the entire correction chunk fails and the corrected lane becomes unavailable. Disabling only word-confidence aggregation lets the same loaded model return the identical decoded-text hash, but the returned hypothesis contains 214 native word-timestamp entries for 213 display words. A bounded structural probe found one display token at index 62 represented by two native timing entries at indices 62-63: their raw concatenation is exact, and the extra entry is punctuation-only. NeMo builds display text and timestamp words through separate decoder paths, so punctuation segmentation can diverge even when the decoded wording is identical. `validated_word_timings()` correctly discards that evidence, so the simple confidence fallback recovers text but not trustworthy timing-dependent behavior.
- **Evidence:** The original corpus-off run and one cold full replay failed on chunk 3/3 with the same pinned Unified model and sanitized `transcribe_failed` signature. A separate cold probe then made exactly one direct call on only the valid 360.0-432.24 second chunk and reproduced `len(words): 213` versus `len(word_confidence): 214` at NeMo's `_aggregate_token_confidence_subwords_sentencepiece`; CUDA remained available after the exception. Preserved as a local-only artifact. The approved two-call recovery probe reused one loaded model, changed only `confidence_cfg.preserve_word_confidence`, and produced an identical text hash with zero word-confidence values and healthy CUDA; its sole failed result gate was the 214/213 timestamp join, which a non-GPU call to the production validator reduced to null. Preserved as a local-only artifact. The follow-up single-call timestamp-alignment probe reproduced the prior display hash and counts, retained exactly one mismatch, and classified it as an exact-concatenation one-to-two token-boundary split with one punctuation-only timestamp entry; all timing bounds remained numeric, ordered, monotonic, non-zero, and within the clip. Preserved as a local-only artifact.
- **Prevention:** Preserve the raw traceback in restricted diagnostic evidence because normal service logs intentionally sanitize clinical text. Diagnose the exact chunk in a fresh process before spending a sequential replay. Any fix must address the exception inside NeMo's transcribe call—not only post-return confidence validation—and must keep GPU-call/retry behavior behind its own approval and acceptance gate. Require both an identical decoded-text fingerprint and an application-valid one-to-one timestamp join; a successful vendor call with present timestamp entries does not prove the correction/rediarization lane can use them. If timestamp reconciliation is proposed, accept only mechanically provable punctuation-only split/merge cases with exact concatenation and monotonic bounds, then rerun the existing strict validator; never drop an arbitrary extra timing row or normalize a lexical substitution.
- **Mitigation status (2026-07-24):** Source, CPU, and isolated pinned-runtime gates now implement and verify that exact boundary. The pinned Unified path classifies only the vendor `RuntimeError` with one extra confidence value and a parseable recognized-text fingerprint, changes only `preserve_word_confidence`, makes one final strict timestamped call, and rejects recovered text whose SHA-256 differs. Native timing rows fold only when raw words concatenate exactly to one display word with one lexical component plus punctuation-only components and finite monotonic bounds; the normal strict validator runs again afterward. Recovery remains unavailable when any chunk lacks application-valid timing. Focused tests passed 88/88, the full Python suite passed 910 with 7 skips, and Ruff passed. The approved application-path GPU run then used one cold-loaded pinned model and exactly two forwarded calls: call 1 reproduced the exact 213/214 classifier, call 2 preserved the expected display hash, the production reconciler made exactly one punctuation-only fold from 214 native rows to 213 application-valid rows, the strict validator accepted all 213 rows, and CUDA remained healthy. The raw observer rejected only because it compared pre-call snapshots and misattributed NeMo's logged `compute_timestamps=true` call side effect to recovery; the preserved adjudication proves recovery owned only `confidence_cfg.preserve_word_confidence`, with all core gates green. Evidence retained locally only. The isolated chunk mitigation is runtime-accepted; the separately gated full replay is recorded below, while corpus promotion remains a separate human gate.
- **Full-replay status (2026-07-24):** The separately approved flag-off replay of the exact 432.24-second d2c09 fixture now accepts the mitigation through the normal WebSocket and correction endpoints. Live output repeated the earlier failed replay's canonical row hash, text hash, 233-row count, and role counts exactly. Correction completed from 233 corrected rows with three planned chunks and exactly four physical calls: one call each for chunks 1 and 2, the classified failure plus one final recovery call for chunk 3, one 214-to-213 punctuation timing fold, zero unaccounted meaningful live rows, and no fifth call. CUDA and the restored flag-off/console service remained healthy. Evidence retained locally only. The failed fixture is full-replay accepted for the confidence-recovery source; corpus promotion and corpus-on remain separate human gates.
...
- **What breaks:** WSL2 can lose its GPU adapter mid-session while everything else keeps
  ...
  available") at 08:35Z after ~25 hot-reloads and two probe processes; the next gate
  eval then ran on CPU against a GPU-captured baseline and produced a false "regression" (c08
...
- **Update (2026-07-11, shipped - evidenced behavior only):**
  ...
  - Acceptance: day5 579s browser replay passed from corrected source (correction 17.06s, chunk_count=4, no retry; summary 32.37s; no notice) (local-only artifact). The 3/3-failing full-length c03 passed on a deliberately dirty GPU: release reclaimed ~4.7 GiB (11354 -> 6688 MiB), bounded peak 11464/16303 MiB vs the fatal 15.7 GiB one-shot spike, 18.09s, attempts=1, then corrected-source summary acceptance (local-only artifact).
...
**Status:** resolved | **Created:** 2026-07-20 | **Evidence:** ACTUAL_MEASURED

- **Files:** `scripts/health-check-localdev.sh` (search: "HEALTH_PROBE_SESSION_ID")
- **Files:** `strands_agents/api/server.py` (search: "async def roles_snapshot")
- **What breaks:** The health script probed `GET /session/test/roles` with the literal ID `test` and required a `session_id` JSON answer. The agent's roles route validates UUIDs and correctly answers 400 for `test`, so the script reported the agent unhealthy and exited 1 on an otherwise fully healthy stack; any gate requiring a literal health pass became unpassable. The PHP proxy check beside it masked the mismatch by wrapping the failure into an empty-shape 200.
- **Evidence:** A 2026-07-20 baseline campaign: the script exited 1 with exactly one failing check while a valid-UUID curl of the same route returned 200. Preserved as a local-only artifact.
- **Resolution (2026-07-20):** Both roles probes now use `HEALTH_PROBE_SESSION_ID` (a fixed valid UUID); the agent route peeks without creating state, so the probe stays side-effect free. Verified live: exit 0 with both probes passing on a healthy stack. Campaign helpers no longer need the substitute-probe workaround for new runs.
