...
- **Evidence:** The original corpus-off run and one cold full replay failed on chunk 3/3 with the same pinned Unified model and sanitized `transcribe_failed` signature. A separate cold probe then made exactly one direct call on only the valid 360.0-432.24 second chunk and reproduced `len(words): 213` versus `len(word_confidence): 214` at NeMo's `_aggregate_token_confidence_subwords_sentencepiece`; CUDA remained available after the exception. Preserved at `var/quality/rediar-m05-acceptance/diagnostics/2026-07-24_d2c09-chunk3-probe1/` (search: "confidence_alignment"). The approved two-call recovery probe reused one loaded model, changed only `confidence_cfg.preserve_word_confidence`, and produced an identical text hash with zero word-confidence values and healthy CUDA; its sole failed result gate was the 214/213 timestamp join, which a non-GPU call to the production validator reduced to null. Preserved at `var/quality/rediar-m05-acceptance/diagnostics/2026-07-24_d2c09-confidence-fallback-probe1/` (search: "APPLICATION-TIMESTAMP-GATE"). The follow-up single-call timestamp-alignment probe reproduced the prior display hash and counts, retained exactly one mismatch, and classified it as an exact-concatenation one-to-two token-boundary split with one punctuation-only timestamp entry; all timing bounds remained numeric, ordered, monotonic, non-zero, and within the clip. Preserved at `var/quality/rediar-m05-acceptance/diagnostics/2026-07-24_d2c09-timestamp-alignment-probe1/` (search: "token_boundary_split_or_merge").
...
- **Evidence:** 2026-07-20 M01 campaign: the script exited 1 with exactly one failing check while a valid-UUID curl of the same route returned 200. Preserved at `var/quality/0.5.1-m01-baseline/attempt-evidence-c02-150-run1-stale-health-gate/`.
