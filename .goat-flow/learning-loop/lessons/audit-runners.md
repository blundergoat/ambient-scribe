...
**What happened:** A one-shot audit shell reaped a bare `nohup` child before replay, while its first
...
A later successor runner declared two proof-only patch variables it never consumed;
...
**Evidence:** Local-only audit artifacts. The recorded signatures were `54/54 citations`
and `SC2034`.
...
**Created:** 2026-07-14
**What happened:** A canonical Ruff lint passed, and its runner passed a focused format check.
An extra repository-wide format check found 34 untouched files outside the milestone's boundary.
**Evidence:** Local-only audit artifacts; the findings above are the record.
...
**Created:** 2026-07-18
**What happened:** A unified-runtime manual-acceptance sealing opened `manifest.tsv` before scanning the evidence root.
...
**Evidence:** Local-only audit artifacts. The recorded signature was
"The first manifest command".
...
**Created:** 2026-07-18
**What happened:** A milestone prepared a default-off context seam and a later conditional prompt rule in one
...
**Created:** 2026-07-19
**What happened:** A GPU-budget check compared a `7728 MiB` total GPU sample with an old `6270 MiB` application
...
**Evidence:** Local-only audit artifacts. The recorded values were
...
**Created:** 2026-07-19
**What happened:** An approved run started the NeMo container in one tool call and launched its sampler in
...
**Evidence:** Local-only audit artifacts. The recorded signature was
`incomplete_missing_early_samples`.
