...
**Status:** Rejected
**Date:** 2026-07-20
**Ticket/Context:** 0.5.1 streaming slot-evidence investigation

## Context

After ADR-008 rejected both sustained-voice fold policies, a deterministic
flag-off baseline was frozen: the reported harmful speaker fold reproduces
...
runs). Behavior-neutral pairwise co-activity counters were then added inside the
...
v0.5.1 fold-candidate line — policy, corpus gates, then promotion — does
not proceed. The harmful-fold repair routes to a separate System plan for
post-stop full-audio speaker re-diarization; live preview keeps
...
Twelve unique fold specimens from evidence-on cold replays of the four baseline
...
The reproducible anchors are the counters and window field cited above. The
derivation packet, join table, and flag-off compatibility run were local-only
evidence artifacts; their measured results are recorded inline here and cannot be
re-read from this repository without a fresh replay.
